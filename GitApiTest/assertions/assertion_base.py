...
def assert_response_body_fields(request, response, exp_obj=None, rmv_ids=True, fields_to_ignore=None):
    """
    проверяет ответ от сервера, сравнивая ожидаемый объект с полученным
    :param fields_to_ignore: поля, по которым не надо валидировать json
    ...
    if fields_to_ignore:
        exp_json = remove_fields(exp_json, fields_to_ignore)
        act_json = remove_fields(response.json(), fields_to_ignore)
